...
public class CloudhubAPICaller {
	...
	public void saveRuntimeAlerts() throws AppException {
		...
		for (String key : Const.RUNTIME_ALERTS) {
			RuntimeAlertResponse response = saveRuntimeAlert(key);
			if (StringUtils.isBlank(response.getId())) {
				throw new AppException("The runtime alert could not be created.");
			}
		}
	}
	
	/**
	 * ランタイムアラート存在チェックと登録処理
	 * 
	 * @return 登録結果
	 * @throws AppException アプリケーション例外
	 */
	public void checkAndSaveRuntimeAlerts() throws AppException {
		...
		CommonResponse res = findRuntimeAlert();
		List<Map<String,Object>> alertlist = res.getData();
		Set<String> alertSet = new HashSet<String>();
		for (Map<String,Object> alertmap : alertlist) {
			alertSet.add((String) alertmap.get("conditionType"));
		}
		for (String key : Const.RUNTIME_ALERTS) {

			if (alertSet.contains(key)) continue;
			RuntimeAlertResponse response = saveRuntimeAlert(key);
			if (StringUtils.isBlank(response.getId())) {
				throw new AppException("The runtime alert could not be created.");
			}
